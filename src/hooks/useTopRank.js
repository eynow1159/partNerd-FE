    ...
    const fetchTopRank = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/api/community/popularity`
        );
            ...
            response.data.result.map(async (item) => {
              ...
              if (item.profileKeyName && item.profileKeyName.trim() !== "") {
                try {
                  ...
                  const presignedUrlResponse = `https://www.partnerd.site/${keyName}`;
                  ...
                  if (
                    presignedUrlResponse.data.isSuccess &&
                    presignedUrlResponse.data.result
                  ) {
                    const cloudFrontUrl = presignedUrlResponse;
                  ...
                  console.warn(
                    "프로필 이미지 URL 생성 실패: 응답 데이터가 올바르지 않음"
                  );
